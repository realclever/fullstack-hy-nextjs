      ...
      {blogs.length === 0 ? (
        ...
        <ul className="grid gap-5">
          {blogs.map((blog) => (
            <li
              key={blog.id}
              className="relative rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-md"
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">
                    <Link
                      href={`/blogs/${blog.id}`}
                      className="before:absolute before:inset-0 transition hover:text-slate-600"
                    >
                      {blog.title}
                    </Link>
                  </h2>
                  ...
                  <a
                    ...
                    className="relative z-10 mt-3 inline-block break-all text-sm font-medium text-blue-600 transition hover:text-blue-800 hover:underline"
                  >
                    {blog.url}
                  </a>
                </div>
