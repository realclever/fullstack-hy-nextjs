          ...
          {users.map((user) => (
            ...
            >
              <Link
                ...
                className="group grid grid-cols-[1.2fr_1fr_5rem] items-center gap-4 px-5 py-4 transition hover:bg-slate-50"
              >
                <span className="font-semibold text-slate-900 transition group-hover:text-slate-600">
                  {user.name}
                </span>
                <span className="text-slate-500">@{user.username}</span>
                <span className="w-12 justify-self-end text-center text-sm font-medium text-slate-700">
                  {user.blogs.length}
                </span>
              </Link>
            </li>
